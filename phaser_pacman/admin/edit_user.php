...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) { // If everything's OK.
		...
		$q = "SELECT user_id FROM jterrazas_entity_pacman_player WHERE email='$e' AND user_id != $id";
		$r = @mysqli_query($dbc, $q);
		if (mysqli_num_rows($r) == 0) {

			// Make the query:
			$q = "UPDATE jterrazas_entity_pacman_player SET first_name='$fn', last_name='$ln', email='$e' WHERE user_id=$id LIMIT 1";
			$r = @mysqli_query ($dbc, $q);
...
$q = "SELECT first_name, last_name, email FROM jterrazas_entity_pacman_player WHERE user_id=$id";		
$r = @mysqli_query ($dbc, $q);
