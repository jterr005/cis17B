...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) { // If everything's OK.
		...
		$q = "SELECT user_id FROM jterrazas_entity_pacman_player WHERE (email='$e' AND pass=SHA1('$p') )";
		...
		if ($num == 1) { // Match was made.
			...
			$q = "UPDATE jterrazas_entity_pacman_player SET pass=SHA1('$np') WHERE user_id=$row[0]";		
			$r = @mysqli_query($dbc, $q);
